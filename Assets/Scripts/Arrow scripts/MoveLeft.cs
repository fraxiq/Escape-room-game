...
public class MoveLeft : MovementController, IPointerClickHandler
{
    private UI ui;
    ...
    public override void Move()
    {
        ...
        {
            arrowObjects[i].transform.Translate(Vector2.left * moveDistance * Time.deltaTime);
        }
        ui.FindSelected();
    }
    private IEnumerator MoveSmoothly(Transform targetTransform)
    {
        Vector3 targetPosition = targetTransform.position + Vector3.left * moveDistance;
        float duration = 0.3f; 
